...
MAX_NUMBER_OF_IMAGES = 2000
MAX_NUMBER_OF_IMAGES_PER_MAP = 500
...
EXPORT_BOUNDING_BOX_IMAGES = True
...
def add_image(_img, _seg, _copy, _boxes, _classes, _index, _progress_str):
    _img = cv2.cvtColor(_img, cv2.COLOR_BGR2RGB)
    cv2.imwrite(f"{DATASET_DIR}/images/{_index}.jpg", _img)
    _copy = cv2.cvtColor(_copy, cv2.COLOR_BGR2RGB)
    cv2.imwrite(f"{DATASET_DIR}/png/{_index}.png", _copy)
...
if __name__ == "__main__":
    ...
    for env_id, target_num_images_curr_map in enumerate(lst_num_images_per_map):
        ...
        while no_images_so_far < MAX_NUMBER_OF_IMAGES and env_step < target_num_images_curr_map:
            ...
            add_image(obs, seg, copy, boxes, classes, no_images_so_far, progress_str)
            no_images_so_far += 1
            env_step += 1
